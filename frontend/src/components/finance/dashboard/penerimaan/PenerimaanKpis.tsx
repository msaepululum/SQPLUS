import { PENERIMAAN_KPIS } from "@/constants/penerimaan-data";
import { cardClassName } from "@/components/ui/Card";

export function PenerimaanKpis() {
  return (
    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 2xl:grid-cols-8">
      {PENERIMAAN_KPIS.map((kpi) => (
        <div key={kpi.label} className={cardClassName({ variant: "default", className: "!p-3" })}>
          <div
            className={`mb-2 flex h-8 w-8 items-center justify-center rounded-lg text-sm ${kpi.iconBg}`}
          >
            {kpi.icon}
          </div>
