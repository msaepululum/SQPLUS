import {
  KAS_BANK_CASH_IN_COMPOSITION,
  KAS_BANK_EXPENSE_TYPES,
...
export function KasBankTrendChart({ filters }: { filters: KasBankFilters }) {
  const data = getKasBankTrendData(filters.periodeTren);
  const max = 140;
  const w = 560;
  const h = 240;
  ...
  const line = (key: "masuk" | "keluar" | "saldoAkhir") =>
    data.map((d, i) => `${i === 0 ? "M" : "L"}${toX(i)},${toY(d[key])}`).join(" ");

  return (
    <div className={cardClassName({ variant: "default" })}>
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-800">Tren Arus Kas 12 Bulan</h3>
      ...
      <div className="mt-2 flex flex-wrap gap-3 text-[0.625rem] text-slate-500">
        <span className="flex items-center gap-1.5"><span className="h-0.5 w-4 bg-emerald-500" /> Kas Masuk</span>
        <span className="flex items-center gap-1.5"><span className="h-0.5 w-4 bg-red-500" /> Kas Keluar</span>
        <span className="flex items-center gap-1.5"><span className="h-0.5 w-4 bg-blue-500" /> Saldo Akhir</span>
      </div>
      <svg viewBox={`0 0 ${w} ${h}`} className="mt-2 w-full" role="img">
        ...
        <path d={line("saldoAkhir")} fill="none" stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" />
        {data.map((d, i) => (
...
export function KasBankCashInDonut() {
  let offset = 0;
  const circles = KAS_BANK_CASH_IN_COMPOSITION.map((s) => {
    const el = (
  ...
  return (
    <div className={cardClassName({ variant: "default" })}>
      <h3 className="text-sm font-semibold text-slate-800">Komposisi Kas Masuk</h3>
      <div className="mt-3 flex flex-col items-center gap-4 sm:flex-row">
        <div className="relative shrink-0">
          <svg viewBox="0 0 44 44" className="h-28 w-28">{circles}</svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-sm font-bold text-slate-900">128,4M</span>
            <span className="text-[9px] text-slate-400">Total</span>
          </div>
        </div>
        <ul className="w-full space-y-1.5">
          {KAS_BANK_CASH_IN_COMPOSITION.map((s) => (
...
export function KasBankExpenseBar({ filters }: { filters: KasBankFilters }) {
  const max = Math.max(...KAS_BANK_EXPENSE_TYPES.map((d) => d.value));
  const unitLabel = KAS_BANK_UNIT_OPTIONS.find((o) => o.value === filters.unit)?.label ?? "Semua Unit";

  return (
    <div className={cardClassName({ variant: "default" })}>
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-800">Pengeluaran Kas per Jenis</h3>
        <span className="shrink-0 rounded-md bg-slate-100 px-2 py-0.5 text-[0.625rem] text-slate-500">{unitLabel}</span>
      </div>
      <ul className="mt-4 space-y-3">
        {KAS_BANK_EXPENSE_TYPES.map((d) => (
          <li key={d.label}>
            <div className="mb-1 flex justify-between text-xs">
              <span className="text-slate-600">{d.label}</span>
              <span className="font-semibold tabular-nums text-slate-800">Rp {d.value.toFixed(1)} M</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-slate-100">
              <div className="h-full rounded-full bg-gradient-to-r from-red-500 to-red-400" style={{ width: `${(d.value / max) * 100}%` }} />
            </div>
          </li>
        ))}
      </ul>
