...
import { SettingsSectionTabs } from "@/components/finance/settings/SettingsSectionTabs";
import {
  BudgetYearScopeBar,
...
import type { SettingsModuleConfig } from "@/constants/settings-pages";

const YEAR_SCOPED_SLUGS = new Set(["mapping-referensi", "parameter-audit"]);
...
export function SettingsModulePage({ config, initialTab }: SettingsModulePageProps) {
  const isYearScoped = YEAR_SCOPED_SLUGS.has(config.slug);

  const inner = (
    <SettingsModulePageInner
      config={config}
...
function SettingsModulePageInner({
  config,
  initialTab,
  isYearScoped,
}: SettingsModulePageProps & { isYearScoped: boolean }) {
