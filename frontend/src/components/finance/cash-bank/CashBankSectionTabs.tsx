"use client";

import type { CashBankSection } from "@/constants/cash-bank-pages";
import { cn } from "@/lib/cn";

type CashBankSectionTabsProps = {
  sections: CashBankSection[];
  labels: Record<string, string>;
  activeId: string;
  onChange: (id: string) => void;
};

export function CashBankSectionTabs({
  sections,
  labels,
  activeId,
  onChange,
}: CashBankSectionTabsProps) {
  return (
    <div className="overflow-x-auto rounded-lg bg-slate-100 p-1 sq-scroll">
      <div className="flex min-w-max gap-0.5">
        {sections.map((section, index) => {
          const isActive = section.id === activeId;
          const label = labels[section.labelKey] ?? section.labelKey;

          return (
            <button
              key={section.id}
              type="button"
              onClick={() => onChange(section.id)}
              className={cn(
                "flex cursor-pointer items-center gap-1.5 rounded-md px-2.5 py-1.5 text-left text-[11px] font-medium transition-colors sm:px-3 sm:text-xs",
                isActive
                  ? "bg-[#0d6e63] text-white shadow-sm"
                  : "bg-white text-slate-600 hover:text-slate-800"
              )}
            >
              <span
                className={cn(
                  "flex h-4 w-4 shrink-0 items-center justify-center rounded text-[10px] font-bold",
                  isActive ? "bg-white/25 text-white" : "text-slate-400"
                )}
              >
                {index + 1}
              </span>
              <span className="whitespace-nowrap">{label}</span>
            </button>
          );
        })}
      </div>
    </div>
  );
}
