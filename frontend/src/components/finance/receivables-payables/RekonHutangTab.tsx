...
import { fetchRekonHutang } from "@/services/hutangPiutangService";
import type { HutangJenisId } from "@/constants/hutang-categories";
...
type RekonHutangTabProps = {
  jenis?: HutangJenisId | "";
};

export function RekonHutangTab({ jenis }: RekonHutangTabProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await fetchRekonHutang(budgetYearId, {
  ...
  if (yearLoading || (loading && rows.length === 0)) {
    return (
      <div className="flex items-center justify-center gap-2 py-14 text-sm text-slate-500">
        ...
        Memuat rekonsiliasi hutang...
      </div>
    );
  }

  if (!budgetYearId || !budgetYear) {
    return <EmptyState title="Pilih tahun anggaran" description="Rekonsiliasi hutang." />;
  }

  return (
    <div className="mt-2 space-y-2">
      <ToolbarShell>
        <ToolbarSearch value={search} onChange={setSearch} placeholder="Cari jurnal, beli, BKU..." />
      ...
      <div className={cn(tableGridShellClassName, "overflow-hidden")}>
        {rows.length === 0 ? (
          <EmptyState title="Tidak ada data" description="Tidak ada jurnal hutang untuk direkonsiliasi." />
        ) : (
          <>
            <Table>
              <THead>
              ...
              <TBody>
                {rows.map((row, i) => (
                  <TR key={`${row.no_jurnal}-${i}`}>
                    ...
                    <TD className="font-mono text-[10px]">{row.no_beli || "—"}</TD>
                    <TD className="font-mono text-[10px]">{row.no_bku || "—"}</TD>
      ...
      <p className={cn(cardClassName({ variant: "flat", className: "!p-2 text-[10px] text-slate-500" }))}>
        Rekonsiliasi jurnal hutang dengan bukti beli (cnobeli) / BKU (cnobku)
      </p>
    </div>
  );
}
